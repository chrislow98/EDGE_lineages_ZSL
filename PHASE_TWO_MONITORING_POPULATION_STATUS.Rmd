...
# load this data: redlist_status, redlist_threats, Generation_length, Pantheria, mammals_pd_merged, IUCN_RL_status_combined, conservation_actions
load(file = "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Data and analysis/Monitoring_population_trends/phase_two_datasets.RData")
...
# Remove DD species from RL analysis- 5109 species
redlist_status <- subset(redlist_status, category!="DD")
...
redlist_status$population_trend[redlist_status$population_trend == "Unknown"] <- "NA"
...
redlist_status_summary <- redlist_status %>% 
  ...
  dplyr::summarise(Richness = length(family), 
                   ...
                   Unknown = sum(is.na(population_trend)),
                   Prop_unknown = Unknown/length(is.na(population_trend)))

######################################### SCRAP CODE- remove after if not using
...
save(mammals_IUCN_full, IUCN_RL_status_combined, RL_change_df, redlist_status, redlist_status_summary, EDGE_mammals, EDGE_mammal_summary, redlist_threats, file = "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Data and analysis/Monitoring_population_trends/RL_population_trend_data_final.RData")
...
save(mammals_IUCN_full, IUCN_RL_status_combined, RL_change_df, redlist_status, redlist_status_summary, EDGE_mammals, EDGE_mammal_summary, redlist_threats_full, redlist_threats, redlist_threat_summary, mammals_IUCN_full, file = "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Data and analysis/Monitoring_population_trends/RL_population_trend_data_final.RData")
...
# trying to highlight key words from conservation action
x <- conservation_actions[grepl("protected areas", conservation_actions[["conservationActions"]]) | grepl("nature reserves", conservation_actions[["conservationActions"]]) | grepl("forest reserve", conservation_actions[["conservationActions"]]) | grepl("national parks", conservation_actions[["conservationActions"]]) | grepl("species occurs", conservation_actions[["conservationActions"]]), ] 

x <- conservation_actions[grepl("occurs in protected area", conservation_actions[["conservationActions"]]) | grepl("found in protected area", conservation_actions[["conservationActions"]]) | grepl("occurs in several protected areas", conservation_actions[["conservationActions"]]) | grepl("occurs in at least one protected area", conservation_actions[["conservationActions"]]),]
...
#load in dataset - 3802 species
Pantheria <- pantheria

# Get mean body size for each family

mammal_bodysize_full <- Pantheria %>% 
  select(Family, Genus, Species, AdultBodyMass_g) %>% 
  group_by(Family) %>% 
  dplyr::summarise(bodysize_mean = mean(AdultBodyMass_g, na.rm = TRUE))
...
save(mammals_pd_merged, EDGE_priority_lineages, EDGE_mammal_summary, EDGE_amphibian_summary, EDGE_bird_summary, EDGE_reptile_summary, file =  "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Data and analysis/EDGE_families_and_scores.EDGE_summary_dataframes_phase_TWO_.RData")
...
mammals_pd_merged$priority_type[is.na(mammals_pd_merged$priority_type)] <- "No"
