...
retrieve_IUCN_data <- function(speciesList){
  ...
  for(sp in speciesList){
    ...
    if (length(UICN_search$result) == 0){
      IUCN_status_sp <- data.frame(Order = 'NA',
                                   Species = sp, 
                                   Status = 'NA', 
                                   Trend = 'NA',
      ...
      IUCN_status_sp <- data.frame(Order = UICN_search$result$order, 
...
IUCN_sub <- c(API_list[300:310])

IUCN_sub

sub <- retrieve_IUCN_data('Arctocephalus australis')

for (i in 301:length(API_list)){
  
  IUCN_population_data <- retrieve_IUCN_data(API_list[i])
}

measure <- rl_measures(name = 'Cerdocyon thous', key = apikey)
