...
# insert current status of Elaphants result into EDGE mammal dataset- based off latest IUCN assessment
carnivore.df <- EDGE_mammals[c("289"),]
...
family.iter <- data.frame(Family = data_out$Family, no.over.median = 0)
for(i in 1:100){
  med.score <- median(data_out[,(i+1)])
  for(j in 1:NROW(family.iter)){
    if(data_out[data_out$Family == family.iter$Family[j],(i+1)] > med.score){
      family.iter$no.over.median[j] <- family.iter$no.over.median[j]+1
    }
  }
}
...
# Create dataframe to store exPD iterations
above_med_EDGE_score <- data.frame(matrix( , ncol = 2, nrow = length(EDGE_mammal_summary$Family)))

# Change column names
colnames(above_med_EDGE_score)<-c("Family", "No_above_median")

# Put vector of family names in dataset 
above_med_EDGE_score[,1] <- EDGE_mammal_summary$Family 

# set i to 1
n <- 1

for (n in 1:NCOL(data_out)){
  score[n] <- median(data_out[,(i+j)])
    
    ifelse(as.vector(as.numeric(data_out[which(data_out$Family == above_med_EDGE_score$Family[n]), c(2:NCOL(data_out))])), data_out$Family[n] < median(NCOL(data_out[n])), 1, 0) 
  
  all_vals[n] <- sum(score[,n])
  
  above_med_EDGE_score$No_above_median <- all_vals[n]
}



ifelse(as.vector(as.numeric(data_out[which(data_out$Family == above_med_EDGE_score$Family[i]), c(2:NCOL(data_out))])) > median(data_out[,i]), 1, 0)

for (i in 1:NROW(above_med_EDGE_score)){
 above_edge_score[i] <- ifelse(data_out[which(data_out$Family == above_med_EDGE_score$Family[i]), < median(NCOL[,2]),1, 0])
