...
renv::init(project = "C:/Users/Student/Documents/UCL/MRes Biodiversity, Evolution and Conservation/Project 2/EDGE_lineages/Github", bare = TRUE)
...
for (i in 2:length(mammal.trees.upham)){
  
  # Add an extra blank column to dataframe for loop
  data_out_upham <- cbind(data_out_upham, data.frame(x = NA))
  
  # Paste in tree name to each column
  colnames(data_out_upham)[NCOL(data_out_upham)] <- paste("tree.", x, sep = "") 
  ...
  for (g in 1:length(UPHAM_family)){
    ...
    # get most recent common ancestor for each list of family
    MRCA_nodes <- MRCA(mammal.trees.upham[[x]], c(UPHAM_family[[g]]$Species))
    
    # Calculate exPD by summing branch lengths of ancestor and descendant branches to MRCA node
    total_expected_PD <- sumEdgeLength(mammal.trees.upham[[x]], ancestors(mammal.trees.upham[[x]], MRCA_nodes, type= "ALL")) + sumEdgeLength(mammal.trees.upham[[x]], descendants(mammal.trees.upham[[x]], MRCA_nodes, type= "ALL"))
    
    # insert k values after first column to stop family name being overridden by exPD
    if(!is.na(total_expected_PD)){
      data_out_upham[g, ncol(data_out_upham)] <- total_expected_PD 
    }else{
      stop(paste("ePD loss score wass not calculated for family ",g," (",data_out_upham$Family[g],") - in tree ",x,sep = ""))
    }
    print(paste("tree", x, "Family", g, sep = "_"))
  }
}
...
test <- merge(EDGE_mammal_summary, ex_PD_full)

test <- test[order(test$assessed_threat_prop, decreasing = TRUE),]

save(data_out, ex_PD_full, mammal.trees.full, mammal.trees.upham, file = "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Data and analysis/Mammal phylogenies/100_family_EDGE_scores.RData")
