...
tab_model(file = "ED_glm_full.html",
  ED_glm_full,
  CSS = list(
...
webshot("ED_glm_full.html", "D_glm_full.png")

# Plotting the response of ED
# pull out the significant effects of the model

ED_effects_5_3 <- effects::effect(term = c("prop_5_3"), mod = EDGE_glm_full) %>%
  # turn it into a dataframe
  as.data.frame()
# Note that at this point, you can also back-transform your values so looking at actual ED and EDGE scores

# set image to save in directory
png(filename = "C:/Users/Student/Zoological Society of London/EDGEofExistence - Chris Low - EDGE lineages/Project II/Results/Intrinsic and extrinsic threatsthreat_5_3_predictor_ED.png", units = "in", width = 4, height = 4, res = 300)
...
ggplot(ED_effects_5_3) + 
  ...
  labs(x = "Proportion of families threatened by logging" ,y = "log(Median ED)")
...
### create tabular format or results
tab_model(file = "EDGE_glm_full.html",
  EDGE_glm_full,
  CSS = list(
    css.depvarhead = 'color: red;',
    css.centeralign = 'text-align: left;', 
...
webshot("EDGE_glm_full.html", "EDGE_glm_full.png")
...
sjPlot::sjp.glm(ED_glm_full, type = "pred", vars = c("prop_8_1", "prop_5_3"), show.ci = T, facet.grid = T)
...
#subset threat data
pop_status_df <- mammals_pd_merged[,c(1,3,9,11,13,42:46,47,49)]
...
correlations <- cor(pop_status_df[,2:10])
corrplot(correlations, method="circle")
...
# different interpreation of model different interaction looking at if the interaction between proportion of LC/NT species predicts population declines
pop_status_dec_glm_DD <- glm(Prop_dec ~ LC_NT_prop*median_ED, family = quasibinomial("logit"), data = pop_status_df)

summary(pop_status_dec_glm_DD)
